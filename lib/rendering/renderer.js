...
function get_texture(key) {
	...
	if(this.icons.has(keylow)) {
		...
		try {
			...
			let did_mipmap = false;
			// excluding firefox from this because fucking firefox lags to shit when I enable this.
			if(this.is_webgl2 && !(navigator.userAgent.includes("Gecko/")) && (width & 1) == 0 && (height & 1) == 0 && width && height) {
				did_mipmap = true;
				let target_level = 0;
				while((width & 1) == 0 && (height & 1) == 0 && width && height && target_level < 5) {
					width = width >> 1;
					height = height >> 1;
					target_level++;
				}
				width = icon.width;
				height = icon.height;
				gl.texParameteri(gl.TEXTURE_2D, gl.TEXTURE_MAX_LEVEL, target_level);
				...
				for(let level = 1; level <= target_level; level++) {
					width = width >> 1;
					height = height >> 1;
					let pixel_size = 1 << level;
					let out_img_width = img.width >> level;
					let out_img_height = img.height >> level;
					let arr = new Uint8ClampedArray(out_img_height * out_img_width * 4);
					for(let y = 0; y < out_img_height; y++) {
						for(let x = 0; x < out_img_width; x++) {
							let r_sum = 0;
							let g_sum = 0;
							let b_sum = 0;
							let a_sum = 0;
							let total = 0;
							// basically I'm gonna average the pixel values together, with the alpha being the weight of the average
							// that way it essentially becomes an average of all the non-transparent pixels.
							// That way super-thin sprites like cables can be seen while zoomed out.
							for(let iy = 0; iy < pixel_size; iy++) {
								for(let ix = 0; ix < pixel_size; ix++) {
									let src_data_offset = (data_width * (y * pixel_size + iy) + (x * pixel_size + ix)) * 4;
									let weight = data.data[src_data_offset+3] / 255;
									total += weight;
									r_sum += weight * data.data[src_data_offset];
									g_sum += weight * data.data[src_data_offset+1];
									b_sum += weight * data.data[src_data_offset+2];
									a_sum += weight * data.data[src_data_offset+3];
								}
							}
							if(total) {
								r_sum = Math.round(r_sum/total);
								g_sum = Math.round(g_sum/total);
								b_sum = Math.round(b_sum/total);
								a_sum = Math.round(a_sum/total);
							}
							let dest_data_offset = (out_img_width * y + x) * 4;
							arr[dest_data_offset] = r_sum;
							arr[dest_data_offset+1] = g_sum;
							arr[dest_data_offset+2] = b_sum;
							arr[dest_data_offset+3] = a_sum;
						}
					}
					// wrapping in uint8 array because the clamped one doesnt work in some browsers here.
					gl.texImage2D(gl.TEXTURE_2D, level, gl.RGBA, out_img_width, out_img_height, 0, gl.RGBA, gl.UNSIGNED_BYTE, new Uint8Array(arr.buffer));
				}
			}

			gl.texParameteri(gl.TEXTURE_2D, gl.TEXTURE_MIN_FILTER, did_mipmap ? gl.LINEAR_MIPMAP_LINEAR : gl.LINEAR);
