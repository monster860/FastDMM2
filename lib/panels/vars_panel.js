...
module.exports = class EditVarsPanel extends Panel {
	...
	constructor(editor, tile, instance) {
		...
		this.content_obj.innerHTML = `<table class='var-table'></table>`;
		let pinned_vars = instance.get_fastdmm_prop("pinned_vars", instance.make_eval_context());
		...
		for(let key of [...this.instance_vars.keys()].sort()) {
			if(instance.type_obj && instance.type_obj.get_var_meta(key)) continue;
			if(!has_error_header) {
		...
		for(let type_obj = instance.type_obj; type_obj != null; type_obj = type_obj.parent) {
		...
		ok_button.addEventListener("click", () => {
			let inst_index = this.tile.contents.indexOf(this.instance);
			if(inst_index != -1) {
