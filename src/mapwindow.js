...
class MapWindow {
	...
	frame(timestamp) {
			...
			for(let z = this.mapwindow_z; z >= 1; z--) {
				for(let x = draw_minx; x <= draw_maxx; x++) {
					for(let y = draw_miny; y <= draw_maxy; y++) {
						...
						if(this.frame_areas) {
							let frame_appearance = tile.get_frame_appearance();
							if(frame_appearance) render_instances.push(new RenderInstance(frame_appearance, x, y, z));
						}
					}
				}
			}
		...
		let status_text = `${this.mouse_x != null ? `${this.mouse_x},${this.mouse_y}` : `??,??`},${this.mapwindow_z}`;
		if(this.active_placement_handler) {
			let handler_text = this.active_placement_handler.get_status_text();
			if(handler_text) {
				status_text += " " + handler_text;
			}
		}
		document.getElementById("statustextwindow").textContent = status_text;
	}
	...
	update_from_mouse(e) {
		this.mouse_x_float = (e.offsetX*devicePixelRatio - (this.canvas.width/2)) / this.mapwindow_zoom/32 + this.mapwindow_x;
		this.mouse_y_float = -(e.offsetY*devicePixelRatio - (this.canvas.height/2)) / this.mapwindow_zoom/32 + this.mapwindow_y;
		this.mouse_x = Math.floor(this.mouse_x_float);
		this.mouse_y = Math.floor(this.mouse_y_float);
		let dmm = this.editor.dmm;
	...
	canvas_mousemove(e) {
		...
		setTimeout(() => {
			...
			if(this.mouse_x && this.mouse_y && this.active_placement_handler) {
				...
				this.active_placement_handler.mousemove_pixel(this.mouse_x_float, this.mouse_y_float);
			}
	...
	canvas_mouseout() {
		...
		this.mouse_x_float = null; this.mouse_y_float = null;
		this.mouse_last_event = null;
	}
