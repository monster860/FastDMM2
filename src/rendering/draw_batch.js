...
class DrawBatch {
	...
	init_buffers() {
		...
		this.buffers = {
			vertices:        {buf: new Float32Array(this.max_vertices*3), size: 3, attrib: "a_position",   gl_buf: null, gl_pos: null, type: gl.FLOAT,         normalize: false},
			uv:              {buf: new Float32Array(this.max_vertices*2), size: 2, attrib: "a_uv",         gl_buf: null, gl_pos: null, type: gl.FLOAT,         normalize: false},
			colors:          {buf: new Float32Array(this.max_vertices*4), size: 4, attrib: "a_color",      gl_buf: null, gl_pos: null, type: gl.FLOAT,         normalize: false},
			pointer_ids:     {buf: new Uint8Array(this.max_vertices*4),   size: 4, attrib: "a_pointerid",  gl_buf: null, gl_pos: null, type: gl.UNSIGNED_BYTE, normalize: true},
			texture_indices: {buf: new Uint8Array(this.max_vertices),     size: 1, attrib: "a_tex_index",  gl_buf: null, gl_pos: null, type: gl.UNSIGNED_BYTE, normalize: false},
		};
	}
	...
	draw() {
		...
		for(let obj of Object.values(this.buffers)) {
			...
			gl.vertexAttribPointer(gl_pos, obj.size, obj.type, obj.normalize, 0, 0);
			gl.enableVertexAttribArray(gl_pos);
		}
		gl.bindFramebuffer(gl.FRAMEBUFFER, null);
		gl.drawArrays(gl.TRIANGLES, 0, this.num_vertices);
		if(this.client.gl_enable_framebuffer) {
			this.assign_uniforms(true);
			gl.bindFramebuffer(gl.FRAMEBUFFER, this.client.gl_click_framebuffer);
